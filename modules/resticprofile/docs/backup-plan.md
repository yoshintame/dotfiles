...
resticprofile использует **cron** (`scheduler: crond` в `global`-секции `profiles.yaml`). Формат расписания — systemd-calendar (`*:00,30` — каждые 30 минут; `2:00` — ежедневно в 02:00) — resticprofile конвертирует его в стандартный cron-формат автоматически.
...
Записи хранятся в пользовательском crontab (`crontab -l`).

**Первичная инициализация crontab** (если crontab пустой — resticprofile не может его прочитать):
...
По умолчанию resticprofile на macOS использует launchd. Launchd-агенты создаются с `LimitLoadToSessionType: Background` — при входе в GUI (сессия Aqua) они **могут не загружаться**: plist-файлы есть, но `resticprofile status` выдаёт `scheduled job not found`, и расписания не работают после перезагрузки.

Cron не зависит от типа сессии и работает стабильно. Для переключения достаточно одной строки в конфиге:
...
- `/usr/sbin/cron` может потребовать **Full Disk Access** — если бэкапы падают с ошибками доступа, добавить в System Settings → Privacy & Security → Full Disk Access
- cron не наследует PATH пользователя — resticprofile записывает в crontab полный путь к бинарнику (`/opt/homebrew/bin/resticprofile`), поэтому PATH не важен
- Формат расписания в `profiles.yaml` остаётся **systemd-calendar** (не cron-синтаксис) — resticprofile конвертирует сам; step-нотация (`*/3`) не поддерживается, нужны явные значения (`0,3,6,...`)
...
cron **не запускает пропущенные задачи** после пробуждения — в отличие от launchd. Если ноутбук спал в момент запланированного запуска, задача будет пропущена.

На практике это приемлемо:

- Dev backup каждые 30 мин — следующий запуск через 30 мин после пробуждения
- Copy каждые 3 часа — при следующем запуске скопирует всё накопившееся
- Maintenance и check — редкие операции, пропуск одного воскресенья/месяца не критичен

Если нужно гарантированное «догоняющее» поведение — можно использовать [`anacron`](https://formulae.brew.sh/formula/anacron) поверх cron. Для текущих задач это излишне.
...
```bash
# Инициализировать crontab если он пустой (нужно только один раз)
echo "" | crontab -

resticprofile schedule --all
# Записывает расписания в crontab пользователя
```
...
# Показывает все активные расписания и статус

crontab -l
# Показывает записи в crontab напрямую
```
...
#### Посмотреть активные cron-записи

```bash
# Все записи resticprofile в crontab
crontab -l | grep resticprofile
```
...
Расписания хранятся в пользовательском crontab (`crontab -l`), секция ограничена маркерами `### this content was generated by resticprofile ###`.
