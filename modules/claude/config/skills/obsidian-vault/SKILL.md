...
- **Слаг персоны** несёт дискриминатор, иначе тёзки коллидируют: `<имя>[-фамилия]-<организация>-<роль>` для людей из организации (`denis-vtb-teamlead`, `alexander-senate-bi`), `<имя>[-фамилия]-<тип отношений>` для семьи и друзей (`nina-smirnova-grandmother`). Роль — короткий латинский токен (`backend`, `pm`, `bi`, `teamlead`, `boss`). `colleague` в слаг не пишут — за него отвечает организация. Если человек и коллега, и друг, выигрывает то, что первично в жизни (`adam-friend`, хотя он же фронтендер VTB). Владелец vault — `mikhail`, без суффиксов. Полное правило: `areas/vault-management/file-naming-convention.md`.
