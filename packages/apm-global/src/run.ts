...
if (!home || !dotfiles || args.length === 0) {
  console.error("Usage: run.ts <apm arguments...>; HOME and DOTFILES must be set");
  process.exit(2);
}
...
async function repairLinks() {
  const needsRelink = (await Promise.all(names.map(persistReplacement))).some(Boolean);
  if (needsRelink) {
    await Promise.all(names.map((name) => rm(join(liveDirectory, name), { force: true })));
...
try {
  ...
  try {
    ...
    repairFailed = true;
    console.error(`Failed to restore global APM links: ${error instanceof Error ? error.message : String(error)}`);
  }
