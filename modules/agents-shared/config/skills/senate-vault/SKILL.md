...
- Коммиты: work-контекст senate, conventional commits, `-S`, атомарно и **позиционно** — перечисляй файлы явно, не `git add -A` / широкий стейдж даже временно (загребёт чужое dirty из параллельных сессий). `git-commit-atomic -C <repo> --auto -S "<msg>" <файлы>`.
