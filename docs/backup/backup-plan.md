...
**Тег:** `home`\
**Частота:** 4 раза в день (00:00, 06:00, 12:00, 18:00)\
**Что входит:**
...
| Home backup        | `home`        | `0,6,12,18:00`           | 4 раза в день     |
...
| -------------------- | -------- | -------- | ------------ |
| `development-backup` | 1 час    | 8 часов  | dev          |
| `home-backup`        | 6 часов  | 8 часов  | home         |
| `copy-to-b2`         | 3 часа   | 10 часов | local-to-b2  |
| `maint-local`        | 7 дней   | 2 дня    | local-ops    |
| `maint-b2`           | 7 дней   | 2 дня    | b2-ops       |

**Почему большие grace.** Ноутбук часто закрыт (sleep), cron на macOS **не догоняет пропущенные задачи** после пробуждения. Поэтому period держим близко к фактическому расписанию (чтобы статистика в HC была честной), а grace подбираем под типичную длину ночного сна ноута. Dead-man-switch всё ещё ловит реальную поломку (8+ часов тишины на dev — уже подозрительно), но обычный ночной сон перестаёт триггерить ложные алерты.

Period/grace задаются в дашборде HC.io, **в `profiles.yaml` их нет** — профайлы знают только UUID пинга. При изменении цифр меняем только UI HC.io.

### TODO: миграция на self-hosted Healthchecks

Запланирован переезд с публичного healthchecks.io на self-hosted инстанс на домашнем homelab. После миграции:

- UUID-ы чеков поменяются → ре-рендер `profiles.tmpl.yaml` через SOPS
- `hc-ping.com` в `run-after`/`run-after-fail` заменится на внутренний URL
- Period/grace переносятся руками или через Management API (этот же API можно будет завернуть в `rp hc sync` для IaC-подхода)
- Telegram-интеграция перенастраивается на новом инстансе
...
## Консистентность: почему нет snapshot'ов

Рассматривалась интеграция APFS local snapshots для консистентного среза ФС на время backup. Реализация сделана и рабочая — лежит в ветке [`apfs-snapshot-integration`](backlog.md#apfs-snapshot-integration-заморожено). **В master не мерджилась** из-за фундаментального ограничения restic 0.18.1: нет способа переопределить `paths` в snapshot metadata, все snapshot'ы получают префикс `/tmp/restic-apfs-snap-*` и ломают UX `rp restore`.

Для dev-машины без больших активных БД риск inconsistency низкий — backup идёт секунды, большинство файлов статичны, shell/git записи атомарны на уровне ФС.

Подробнее: [backlog.md](backlog.md).

## См. также

- [Backup backlog](backlog.md) — планы и отложенные идеи
