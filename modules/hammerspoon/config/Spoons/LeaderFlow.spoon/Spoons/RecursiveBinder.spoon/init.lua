...
---    1) A single key spec: { {mods}, key }
---    2) Multiple key specs: { { {mods}, key }, { {mods}, key }, ... }
---  * Default to {keyNone, 'escape'}
...
--- RecursiveBinder.abortOnMouseClick
--- Variable
--- whether to abort/exit when any mouse button is clicked while the modal is active.
---
--- Notes:
---  * Default to false.
---  * The click event is passed through (not swallowed).
obj.abortOnMouseClick = false
...
function obj.recursiveBind(keymap, modals)
   ...
   local function stopMouseAbortTap()
      if obj._mouseAbortTap then
         obj._mouseAbortTap:stop()
         obj._mouseAbortTap = nil
      end
   end

   local function startMouseAbortTap()
      stopMouseAbortTap()
      if not obj.abortOnMouseClick then return end

      obj._mouseAbortTap = hs.eventtap.new({
         ...
         killHelper()
         stopMouseAbortTap()
         return false -- pass the click through
      end)

      obj._mouseAbortTap:start()
   end

   local function normalizeEscapeKeys(escapeKey)
      -- Backward compatible:
      -- - single: { {mods}, key }
      -- - multi:  { { {mods}, key }, { {mods}, key }, ... }
      if not escapeKey then return { { keyNone, 'escape' } } end
      if type(escapeKey) ~= 'table' then return { { keyNone, 'escape' } } end

      if type(escapeKey[1]) == 'table' and type(escapeKey[2]) == 'string' then
         -- single spec
         return { escapeKey }
      end

      -- assume list of specs
      return escapeKey
   end
   ...
   for _, ek in pairs(normalizeEscapeKeys(obj.escapeKey)) do
      if type(ek) == 'table' and type(ek[1]) == 'table' and type(ek[2]) == 'string' then
         modal:bind(ek[1], ek[2], function()
            stopMouseAbortTap()
            modal:exit()
            killHelper()
         end)
