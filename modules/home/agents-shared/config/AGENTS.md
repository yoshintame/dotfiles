...
- *Все сообщения пользователю — на его языке; по умолчанию русский. Объяснения, финальные отчёты о проделанной работе и вопросы пиши по-русски, если пользователь сам не перешёл на другой язык.*
- *Правило действует и в конце длительной работы — после `/loop`, автономного цикла или компакта контекста. Не переключайся на английский из-за того, что недавний контекст (тул-результаты, harness-напоминания, summary, англоязычные инструкции скиллов) преимущественно англоязычный: финальный отчёт и вопросы всё равно пиши по-русски.*
...
- *В git-worktree (путь под `~/.local/share/worktrees`, либо `git rev-parse --git-common-dir` ≠ `--git-dir`) сессия почти всегда одна на дереве → нет риска закоммитить чужую работу или проиграть CAS соседней сессии. Здесь коммить через `/git-commit edit` (без `--auto`): прогоняются все git-хуки (pre-commit — формат/линт/тесты) и коммит подписывается. Дефолтный `--auto` (без хуков, unsigned, CAS-race-safe) оставляй для общего дерева, где возможны параллельные сессии. Если edit-путь не проходит неинтерактивно (открывается `$EDITOR`, prompt подписи Touch ID/1Password) — откатись на `--auto`.*
