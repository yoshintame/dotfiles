...
async function fixture(mode: 'success' | 'failure' | 'interrupt') {
	...
	await Promise.all([
		chmod(join(bin, 'apm'), 0o755),
		chmod(join(bin, 'mise'), 0o755),
	])
...
async function expectRepaired(paths: Awaited<ReturnType<typeof fixture>>) {
	...
	expect(await readFile(join(paths.canonical, 'apm.lock.yaml'), 'utf8')).toBe(
		'version: next\n',
	)
	expect((await lstat(join(paths.live, 'apm.yml'))).isSymbolicLink()).toBeTrue()
	expect(
		(await lstat(join(paths.live, 'apm.lock.yaml'))).isSymbolicLink(),
	).toBeTrue()
	expect(await readlink(join(paths.live, 'apm.yml'))).toBe(
		join(paths.canonical, 'apm.yml'),
	)
	expect(await readlink(join(paths.live, 'apm.lock.yaml'))).toBe(
		join(paths.canonical, 'apm.lock.yaml'),
	)
}
...
describe('global APM wrapper', () => {
	...
	test('repairs links after interruption', async () => {
		...
		await Bun.file(join(paths.root, 'ready'))
			.exists()
			.then(async (ready) => {
				while (!ready) {
					await Bun.sleep(10)
