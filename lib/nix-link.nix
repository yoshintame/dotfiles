    ...
    let
      walk = rel:
        ...
        in lib.concatMap (name:
          let sub = if rel == "" then name else "${rel}/${name}";
          in if lib.elem name pruneNames then [ ]
             else if entries.${name} == "directory" then walk sub
             else [ sub ]
  ...
  linkFiles = dest: value:
    if builtins.isString value then
      ...
      in lib.listToAttrs (map
        (rel: lib.nameValuePair "${toKey dest}/${rel}" (entry "${liveBase}/${rel}"))
...
in {
  options.nixDotbot = {
    ...
    dotfilesDir = lib.mkOption { type = lib.types.str; default = "~/.dotfiles"; };
    links = lib.mkOption { type = lib.types.attrsOf lib.types.anything; default = {}; };
    defaults = lib.mkOption { type = lib.types.attrsOf lib.types.anything; default = {}; };
    clean = lib.mkOption { type = lib.types.listOf lib.types.str; default = []; };  # no-op
  };
